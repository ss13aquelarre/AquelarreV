...
/datum/chatmessage/proc/split_text_preserving_html(text)
    var/list/result = list()
    var/len = length_char(text)
    var/i = 1

    while(i <= len)
        var/char = copytext_char(text, i, i)

        if(char == "&")
            var/entity_end = findtext(text, ";", i)
            if(entity_end)
                var/entity_len = length_char(copytext_char(text, i, entity_end))
                result += copytext_char(text, i, i + entity_len - 1)
                i += entity_len
                continue

        if(char == "<")
            var/tag_end = findtext(text, ">", i)
            if(tag_end)
                var/tag_len = length_char(copytext_char(text, i, tag_end))
                result += copytext_char(text, i, i + tag_len - 1)
                i += tag_len
                continue
